...
plot_interval <- function(data, var, trim = 0.99, xlab = "days", ylab = "cases") {
  ...
  ggplot(data, aes({{ var }})) +
    geom_histogram(binwidth = 1, boundary = -0.5, fill = "grey80", colour = "white") +
...
```{r}
readRDS2 <- function(file) readRDS(file.path(path2data, file))
```
...
```{r}
df_census <- readRDS2(census_file)
```
...
df_ch2 <- case_ch2 |> 
  readRDS2() |> 
  filter(year(admission) %in% c(2024, 2025))
```
...
```{r}
df_ch1_out <- readRDS2(ch1_out_file)
```
